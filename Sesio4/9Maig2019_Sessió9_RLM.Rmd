...
```{r}
m21<-lm(log(duration)~poly(emp.var.rate,3),data=df)
summary(m21)
Anova(m21)

m21<-lm(log(duration)~emp.var.rate+I(emp.var.rate^2)+I(emp.var.rate^3),data=df)
summary(m21)
```
...
m1<-lm(prestige~income+education+type,data=Prestige)
scatter3d(prestige~income+education|type,data=Prestige)
summary(m1)
vif(m1)

m2<-lm(prestige~poly(income,2)+poly(education,2)+type,data=Prestige)
summary(m2)
Anova(m2)
anova(m1,m2)

m3<-lm(prestige~poly(income,2)+education+type,data=Prestige)
summary(m3)
Anova(m3)
anova(m1,m3)
anova(m3,m2)
